...
# set _RESPONSE_HEADERS as dict
_RESPONSE_HEADER_DICT = dict(zip(map(lambda x: x.upper(), _RESPONSE_HEADERS), _RESPONSE_HEADERS))
# print _RESPONSE_HEADER_DIC
# {'ALLOW': 'Allow', 'CONTENT-LANGUAGE': 'Content-Language', 'PROXY-AUTHENTICATE': 'Proxy-Authenticate', 'AGE': 'Age'
# ......
...
class HttpError(Exception):
    ...
    def __init__(self, code):
        """
        Initialize an HttpError with response code.
        """
...
def _build_regex(path):
    ...
    var_list = []
    is_var = False
    for v in _re_route.split(path):     # re.split: Split string by the occurrences of pattern.
        if is_var:
            var_name = v[1:]
...
class Route(object):
    """
    A Route object is a callable object. 路由对象
    """
    def __init__(self, func):
        self.path = func.__web_route__  # 路由对象的路径
        self.method = func.__web_method__   # 路由对象的方法
        self.is_static = _re_route.search(self.path) is None
        if not self.is_static:
            self.route = re.compile(_build_regex(self.path))
        self.func = func    # 路由对象的函数
...
class Request(object):
    ...
    def _parse_input(self):
        ...
        FieldStorage: Store a sequence of fields, reading multipart/form-data
          ...
          keep_blank_values: flag indicating whether blank values in percent-encoded forms should be treated as blank strings.
        """
    ...
    def set_cookie(self, name, value, max_age=None, expires=None, path='/', domain=None, secure=False, http_only=True):
        ...
        if expires is not None:
            if isinstance(expires, (float, int, long)):
                # example: Wed, 21-Dec-2011 09:09:10 GMT
...
class Jinja2TemplateEngine(TemplateEngine):
    ...
    def __init__(self, templ_dir, **kw):
        ...
        if 'autoescape' not in kw:
            kw['autoescape'] = True     # autoescape: 转义(转义其实就是把HTML代码给转换成HTML实体了)
...
def _build_pattern_fn(pattern):
    ...
    if m:
        # Return a function which judge the string whether start with m.group(1)
        return lambda p: p.startswith(m.group(1))
    m = _RE_INTERCEPTROR_ENDS_WITH.match(pattern)
    if m:
        # Return a function which judge the string whether end with m.group(1)
...
def interceptor(pattern='/'):
    ...
    """
    def _decorator(func):
        # Get a function which judge the string whether start/end with m.group(1) for func
...
def _build_interceptor_chain(last_fn, *interceptors):
    ...
    fn = last_fn
    for f in L:
        fn = _build_interceptor_fn(f, fn)   # Build interceptors for fn (Insert fn at the end of f)
    return fn
...
def _load_module(module_name):
    ...
    >>> m.__name__
    """
    last_dot = module_name.rfind('.')  # rfind: Return the highest index in s where substring sub is found
    if last_dot == (-1):
...
class WSGIApplication(object):
    ...
    def get_wsgi_application(self, debug=False):
        ...
        def fn_route():  # Search path_info in 路由表(_get_static,_get_dynamic)?
        ...
        # Insert self._interceptors before fn_route
