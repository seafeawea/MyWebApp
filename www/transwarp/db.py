...
def next_id(t=None):
    ...
    :param t: unix timestamp, default to None and use time.time()
    """
    if t is None:
        t = time.time()
...
def with_transaction(func):
    """
    A decortator that makes function around transaction.
...
def _select(sql, first, *args):
    ...
    sql:
        sql statements.
    first:
        True: fetch one result.
        False: fetch list results.

    """
    global _db_ctx
    cursor = None
    sql = sql.replace('?', '%s')    # Replace char '?' with '%s'
...
if __name__=='__main__':
    ...
    create_engine('xz', 'interact', 'test')
