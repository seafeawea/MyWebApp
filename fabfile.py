...
env.user = 'root'
env.sudo_user = 'root'
env.hosts = ['121.42.137.234']
env.password = 'seaFeawea302139'

db_user = 'root'
db_password = '270573466'
...
def rollback():
    ...
    with cd(_REMOTE_BASE_DIR):
        ...
        if len(files) == index + 1:
            print ('ERROR: already the oldest version.')    # The oldest version cannot rollback
        old = files[index + 1]
        print ('==================================================')
        for f in files:  # for all the versions in server
            if f == current:
